...
class NodeInstaller implements InstallerInterface
{
    ...
    {
        $process = new Process(["tar -xvf " . $source . " -C ".escapeshellarg($targetDir) . " --strip 1"]);
        $process->run();

        if (!$process->isSuccessful()) {
            throw new \RuntimeException(sprintf(
                'An error occurred while untaring NodeJS (%s) to %s: ' . "tar -xvf " . $source . " -C ".escapeshellarg($targetDir) . " --strip 1",
                $source,
                $targetDir
            ));
        }
